"""
File: octobot/laws/validator.py
Fix Type: Security / Governance
Summary:
- ✅ Fixed: validator crashing on malformed impact.json and inconsistent coverage
- ✅ Added: validate_json helper with violation logging
- ✅ Tested by: tests/test_validator_resilience.py

The validator enforces that proposals meet constitutional obligations while
remaining resilient to malformed artefacts.  Coverage is normalised to fractions
between 0 and 1 to keep calculations consistent across the system.
"""
...
def validate_json(path: Path, issues: List[str], namespace: str) -> Dict[str, Any]:
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        message = f"{namespace}: malformed JSON ({error})"
        issues.append(message)
        log_event("laws", "json_decode", "violation", {"file": str(path), "error": str(error)})
        return {}
    if not isinstance(raw, dict):
        issues.append(f"{namespace}: expected an object")
        return {}
    return raw


def _normalise_coverage(raw_value: Any) -> float:
    try:
...
def validate_proposal(proposal: "Proposal") -> ValidationReport:
    ...
    coverage = _normalise_coverage(metadata.get("coverage")) if metadata else 0.0

    quality = _load_quality_gates()
    min_coverage = _normalise_coverage(quality.get("min_coverage", 0.0))
    ...
    if not impact_path.exists():
        ...
        impact = validate_json(impact_path, issues, "impact.json")
